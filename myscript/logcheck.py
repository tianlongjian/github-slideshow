...
def checklog(logtag,logpath):
    ...
    buildtime="[ro.mba.buildtime]"
    buildversion="[ro.custom.build.version]"
    sp2=None
    sp3=None
    for line in file.readlines():
        if logtag == "main":
            ...
            if SipProcess in line:
                #print(line)
                sm4=re.sub("\[\d+:\d+\]","",line)               
                sm3=re.findall("SIPTX: \[SIPTX-IO\](.*)",sm4)
        ...
        if logtag == "prop":  
            ...
            if buildtime in line:
                sp5=re.findall(r"\[ro.mba.buildtime\]: \[(.*)\]",line)
                #print(line)
                print(buildtime,sp5)
            if buildversion in line:
                #print(line)
                sp6=re.findall(r"\[ro.custom.build.version\]: \[(.*)\]",line)
                print(buildversion,sp6)
